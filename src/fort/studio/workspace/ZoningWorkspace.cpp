...
void ZoningWorkspace::setSceneMode(VectorialScene::Mode mode) {
	...
	d_vectorialScene->setColor(Conversion::colorFromFM(fm::DefaultPaletteColor(currentZoneID())));
...
void ZoningWorkspace::onComboBoxCurrentIndexChanged(int) {
	...
	d_vectorialScene->setColor(Conversion::colorFromFM(fmp::DefaultPaletteColor(zoneID)));
...
void ZoningWorkspace::changeShapeType(Shape * shape, fmp::Zone::ID zoneID) {
	...
	fi->first->setColor(Conversion::colorFromFM(fmp::DefaultPaletteColor(zoneID)));
	rebuildGeometry(zoneID);
